...
def num_sentences(txt):
    ...
    comma_list = [',', ';']
    not_EOS_pos_tags = ["DT","IN", "CC","DT", "PRP$", "WP$", "IN", "VB", "VBP", "VBZ", "VBD"]
    ...
    for i, s in enumerate(tokenized_sentences):
        ...
        for i2, w in enumerate(POS_tags):
            if i2 == 0:
            ...
            if word[0].isupper() and tag != 'NNP' and word_before not in comma_list and tag_before not in not_EOS_pos_tags:
                ...
                # tokenized_sentences_copy.remove(s)
                holder_sentence = ""
    ...
    sub_coord_tags = ['CC', 'IN', 'WP', 'WDT', 'WP$', 'RB']
    ...
    for i, s in enumerate(new_sentences):
        ...
        #If the there are more finite verbs than counted sub/coord clauses, it should indicate a missed sentence
        if sum_sub_coord_tags < num_finite_verbs:
            #add 1 as offset for corrent missed count if word includes an "and"
            if sum_and_words != 0:
                sum_and_words += 1
        ...
        # print()
        # print(f"num_finite: {num_finite_verbs} || SCV: {sum_sub_coord_tags} || num_missed {(num_finite_verbs-sum_sub_coord_tags-1)}|| {s}")
        # print(POS_tags)
    ...
    # print(f"num sentences: {num_sentences} || tokenized_senteces: {tokenized_sentences_len} || new_sentence: {len(new_sentences)} || num_missed: {num_missed_sentences}")
